...
func (p *peer) incrPlus(data int) (result int) {
	c := make(chan struct{})
	p.actions <- func() {
		defer close(c)
		st := p.st.incrPlus(data)
		if p.send != nil {
			p.send.GossipBroadcast(st)
		} else {
			p.logger.Printf("no sender configured; not broadcasting update right now")
...
func (p *peer) Gossip() (complete mesh.GossipData) {
	complete = p.st.copy()
	p.logger.Printf("Gossip => complete %v . value -> %v", complete.(*state).set, p.get())
	return complete
}
...
func (p *peer) OnGossip(buf []byte) (delta mesh.GossipData, err error) {
	...
	if delta == nil {
		p.logger.Printf("OnGossip %v => delta %v . value -> %v", set, delta, p.get())
	} else {
		p.logger.Printf("OnGossip %v => delta %v . value -> %v", set, delta.(*state).set, p.get())
	}
	return delta, nil
}
...
func (p *peer) OnGossipBroadcast(src mesh.PeerName, buf []byte) (received mesh.GossipData, err error) {
	...
	if received == nil {
		p.logger.Printf("OnGossipBroadcast %s %v => delta %v . value -> %v", src, set, received, p.get())
	} else {
		p.logger.Printf("OnGossipBroadcast %s %v => delta %v . value -> %v", src, set, received.(*state).set, p.get())
	}
	return received, nil
}
...
func (p *peer) OnGossipUnicast(src mesh.PeerName, buf []byte) error {
	...
	p.logger.Printf("OnGossipUnicast %s %v => complete %v . value -> %v", src, set, complete, p.get())
	return nil
}
